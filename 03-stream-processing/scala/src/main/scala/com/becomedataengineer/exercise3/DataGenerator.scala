...
object DataGenerator {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topicName = "visits_raw_for_dlq"
    Seq(topicName, "dead_letter", "valid_data").foreach(topic => {
      Try {
        val cmdDelete = s"docker exec p3_kafka kafka-topics.sh --bootstrap-server localhost:29092 --topic ${topic} --delete"
        val outputDelete = cmdDelete.!!
      }
      val cmdCreate = s"docker exec p3_kafka kafka-topics.sh --bootstrap-server localhost:29092 --topic ${topic} --create --partitions 2"
